...
def test_main():
    mydict = SqliteDict('data.db', 'user', autocommit=True)
    mydict['some_key2'] = {'23': 12323, 'a': 231}
    for key, vvalue in mydict.items():
        print(key, vvalue)
    pprint(mydict)
    tables = mydict.get_tablenames('data.db')
    print(tables)
    # mydict.commit()
...
def sqlite3_test():
    ...
    # 插入数据
    # 一次插入一条数据
    mySqlite.operate_one('insert into info values(?,?,?,?)', (4, 'Tom3', 22, 'abc'))
    # 一次插入多条数据
    ...
    # 创建游标
    cur = conn.cursor()
    result_one = cur.execute("select * from info where name=:myname ", {"myname": 'Tom3'})
    print(result_one.fetchall())
    print(result_one)
    result_many = mySqlite.query_many(select_sql, (23, 'Tom'))
    print(result_many)
    result_all = cur.execute("select * from info")
    print(result_all.fetchall())
    # 删除数据
    '''
